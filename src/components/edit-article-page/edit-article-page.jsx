...
const EditArticle = ({article, editStatus, token, EDIT_ARTICLE}) => {
  ...
  const deleteTag = (tag) => {
    setArticleLocal({
      article: {
        ...articleLocal.article,
        tagList: [...articleLocal.article.tagList.filter(el => {
          return el !== tag
        })]
      }
    })
  ...
  const tagList = articleLocal.article.tagList.length > 0 ?
    <>
      <div className={classes['tag-item']}>
        {articleLocal.article.tagList.map(el => {
          return (
            <>
              <input className={classes['edit-article-tag__textfield']} type="text" size="40" placeholder="Tag" value={el}/>
        ...
        <input className={classes['edit-article-tag__textfield']} type="text" size="40" placeholder="Tag" onChange={onChangeTagField} value={tag}/>
    ...
    :
    <div className={classes['tag-item']}>
      <input className={classes['edit-article-tag__textfield']} type="text" size="40" placeholder="Tag" onChange={onChangeTagField}/>
      <button className={classes['delete-button']} type="button">
        <span>Delete</span>
      </button>
      <button className={classes['add-button']} type="button" onClick={() => {addTag(tag)}}>
  ...
  return (
    ...
    <div className={classes['edit-article__wrapper']}>
      <form action={() => {}}>
        ...
        <button className={classes['send-button']} type="button" onClick={() => {onSubmit()}}>
          <span>Send</span>
        </button>
      </form>
    </div>
